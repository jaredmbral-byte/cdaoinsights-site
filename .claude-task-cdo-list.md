# Task: New CDO Onboarding List — Export API + Landing Page

## OVERVIEW

Build a weekly "New CDO Appointments" export that vendors can purchase. When a new CDO is appointed, they have ~90 days before their first major vendor decisions. This list gives vendors a head start.

---

## TASK 1: Create export API endpoint

Create `app/api/export/new-cdo-list/route.ts`:
...
## TASK 2: Create /intelligence/new-cdo-list landing page

Create `app/intelligence/new-cdo-list/page.tsx` — a public-facing landing page that shows a PREVIEW of the list (last 7 days, 5 rows max) with a CTA to get the full weekly list.
...
## TASK 3: Add nav link to the new page

In `app/layout.tsx`, in the existing nav, the "Moves" link currently goes to `/moves`. After it, add a subtle link. Actually — don't add to main nav (too crowded). Instead, on the `/moves` page (`app/moves/page.tsx`), add a small callout banner at the top linking to `/intelligence/new-cdo-list`. Like:

After the page header `<p>` and `<h1>` block, add:
```tsx
{/* Product callout */}
...
## TASK 4: Add EXPORT_SECRET to .env.example

Add to `.env.example` (create if doesn't exist):
```
EXPORT_SECRET=your-secret-token-here
```
...
```bash
git add -A
git commit -m "feat: New CDO appointment list — export API + landing page + moves page callout"
git push origin main
```

Then: `openclaw system event --text "Done: New CDO onboarding list live — /intelligence/new-cdo-list landing page + export API at /api/export/new-cdo-list. Pushed to main." --mode now`
