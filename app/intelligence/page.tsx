...
const TOPIC_META: Record<string, { label: string; color: string }> = {
  ai: { label: 'AI', color: 'border-[#C9C4BB] text-[#6B6864]' },
  genai: { label: 'GenAI', color: 'border-[#C9C4BB] text-[#6B6864]' },
  governance: { label: 'Governance', color: 'border-[#C9C4BB] text-[#6B6864]' },
  strategy: { label: 'Strategy', color: 'border-[#C9C4BB] text-[#6B6864]' },
  leadership: { label: 'Leadership', color: 'border-[#C9C4BB] text-[#6B6864]' },
  funding: { label: 'Funding', color: 'border-[#C9C4BB] text-[#6B6864]' },
  'data-quality': { label: 'Data Quality', color: 'border-[#C9C4BB] text-[#6B6864]' },
  security: { label: 'Security', color: 'border-[#C9C4BB] text-[#6B6864]' },
  'agentic-ai': { label: 'Agentic AI', color: 'border-[#C9C4BB] text-[#6B6864]' },
  infrastructure: { label: 'Infrastructure', color: 'border-[#C9C4BB] text-[#6B6864]' },
  layoffs: { label: 'Layoffs', color: 'border-[#C9C4BB] text-[#6B6864]' },
  'ai-deployment': { label: 'AI Deployment', color: 'border-[#C9C4BB] text-[#6B6864]' },
  'microsoft-fabric': { label: 'Microsoft Fabric', color: 'border-[#C9C4BB] text-[#6B6864]' },
  'enterprise-ai-tools': { label: 'AI Tools', color: 'border-[#C9C4BB] text-[#6B6864]' },
  general: { label: 'General', color: 'border-[#8A8782] text-[#6B6864]' },
}
...
export default async function IntelligencePage({
  ...
  return (
    <main className="flex-1 max-w-[1200px] mx-auto px-6 pt-10 pb-24 w-full">
      {/* Compact header */}
      <p className="font-mono text-xs font-medium tracking-[2px] uppercase text-[#6B6864] mb-2">
        Market Intelligence
      </p>
      <h1 className="text-2xl sm:text-3xl font-semibold leading-[1.15] tracking-[-0.5px] text-[#0A0A0A] mb-2">
        Signal Dashboard
      </h1>
      <p className="text-sm text-[#6B6864] leading-relaxed max-w-2xl mb-6">
      ...
      {dedupedArticles[0]?.published_at && (
        <p className="font-mono text-[10px] text-[#6B6864] mt-1 mb-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-4">
        ...
        <div className="space-y-4">
          ...
          <div className="border border-[#C9C4BB] rounded-sm p-4">
            <h2 className="font-mono text-[10px] uppercase tracking-[2px] text-[#6B6864] mb-3">
              Topics
            </h2>
            {/* All filter */}
            <a
              href="/intelligence"
              className={`flex items-center justify-between py-1.5 border-b border-[#C9C4BB] text-xs transition-colors ${
                !activeTopic ? 'text-[#0A0A0A]' : 'text-[#6B6864] hover:text-[#0A0A0A]'
              }`}
            >
            ...
            {sortedTopics.map(([topic, count]) => {
              ...
              return (
                <a
                  ...
                  className={`flex items-center justify-between py-1.5 border-b border-[#C9C4BB] last:border-0 text-xs transition-colors ${
                    activeTopic === topic ? 'text-[#0A0A0A]' : 'text-[#6B6864] hover:text-[#0A0A0A]'
                  }`}
                >
          ...
          <div className="border border-[#C9C4BB] rounded-sm p-4">
            ...
            {sortedSources.map(([source, count]) => (
              <div
                key={source}
                className="flex items-center justify-between py-1.5 border-b border-[#C9C4BB] last:border-0"
              >
                <span className="text-xs text-[#6B6864] truncate mr-2">{source}</span>
                <span className="font-mono text-[11px] text-[#6B6864] flex-shrink-0">{count}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          {activeTopic ? (
            ...
            <>
              <div className="flex items-center justify-between px-4 py-3 border-b border-[#C9C4BB]">
                ...
                <span className="font-mono text-[10px] text-[#6B6864]">
              ...
              {dedupedArticles.length === 0 ? (
                <div className="p-8 text-center">
                  <p className="text-xs text-[#6B6864]">No signals found for this topic. Try a different filter.</p>
                </div>
              ) : (
            ...
            <>
              ...
              {dedupedArticles.length === 0 ? (
                <div className="p-8 text-center">
                  <p className="text-xs text-[#6B6864]">No signals available.</p>
                </div>
              ) : (
                <div>
                  {TOPIC_DISPLAY_ORDER.map(topic => {
                    ...
                    return (
                      <div key={topic} className="mb-6 last:mb-0">
                        {/* Topic header */}
                        <div className="flex items-center justify-between px-4 py-2 border-b border-[#C9C4BB]">
                          <span className={`font-mono text-[10px] uppercase tracking-[2px] px-2 py-0.5 rounded-sm border ${meta.color}`}>
                            {meta.label}
                          </span>
                          <a href={`/intelligence?topic=${topic}`} className="font-mono text-[10px] text-[#6B6864] hover:text-[#0A0A0A] transition-colors">
                            View all →
                          </a>
                        </div>
                        ...
                        {topicArticles.map(article => (
                          <a key={article.id} href={article.source_url} target="_blank" rel="noopener noreferrer"
                            className="block px-4 py-2.5 hover:bg-[#EDEAE2] transition-colors group border-b border-[#C9C4BB] last:border-0">
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0 flex-1">
                                <h3 className="text-sm text-[#0A0A0A] group-hover:text-[#0A0A0A] leading-snug line-clamp-1">
                                  {cleanTitle(article.title)}
