...
export default async function SponsorsPage({
  ...
  return (
    <main className="flex-1 max-w-[1200px] mx-auto px-6 pt-16 pb-24 w-full">
      ...
      <p className="text-base text-[#6B6864] leading-relaxed max-w-2xl mb-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-16">
        {STATS.map((stat) => (
          <div key={stat.label} className="border border-[#C9C4BB] rounded-sm p-4 text-center">
            <p className="text-2xl font-semibold text-[#0A0A0A] mb-1">{stat.value}</p>
            <p className="font-mono text-[10px] uppercase tracking-[1px] text-[#6B6864]">{stat.label}</p>
          </div>
        ))}
      </div>
      ...
      <section className="mb-16">
        <h2 className="font-mono text-xs font-medium tracking-[2px] uppercase text-[#6B6864] mb-6">
          Sponsorship Tiers
        </h2>
          ...
          {TIERS.map((tier) => (
            <div
              ...
              className={`border rounded-sm p-6 flex flex-col ${
                tier.highlight ? 'border-[#0A0A0A]/40 bg-[#0A0A0A]/[0.03]' : 'border-[#C9C4BB]'
              }`}
            >
              <h3 className="font-mono text-xs uppercase tracking-[2px] text-[#6B6864] mb-2">{tier.name}</h3>
              <p className="text-2xl font-semibold text-[#0A0A0A] mb-1">
                {tier.price}
                <span className="text-sm font-normal text-[#6B6864]">{tier.period}</span>
              </p>
              <ul className="mt-4 space-y-2 flex-1">
                {tier.features.map((feature) => (
                  <li key={feature} className="text-sm text-[#6B6864] leading-relaxed flex items-start gap-2">
                    <span className="text-[#0A0A0A] mt-0.5 flex-shrink-0">&bull;</span>
                    {feature}
                  </li>
                ))}
              </ul>
      ...
      <section className="mb-16">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {CATEGORIES.map((cat) => (
            <div key={cat} className="border border-[#C9C4BB] rounded-sm px-4 py-3 flex items-center justify-between">
              <span className="text-sm text-[#0A0A0A]">{cat}</span>
              <span className="font-mono text-[10px] uppercase tracking-[1px] px-2 py-0.5 rounded-sm border border-[#0A0A0A]/30 text-[#0A0A0A]">
                Open
              </span>
            </div>
          ))}
      ...
      {/* CTA */}
      <section className="border border-[#C9C4BB] rounded-sm p-8 text-center">
        <h2 className="text-xl font-semibold text-[#0A0A0A] mb-3">Ready to reach enterprise data leaders?</h2>
        <p className="text-sm text-[#6B6864] mb-6 max-w-lg mx-auto">
